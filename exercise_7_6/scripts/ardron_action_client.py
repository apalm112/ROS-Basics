...
""" 
Exercise 7.6:
$ roslaunch exercise_7_6 ardron_action_client.launch
$ roslaunch ardrone_as action_server.launch
$ rostopic pub /ardrone_action_server/goal ardrone_as/ArdroneActionGoal [TAB][TAB]
	-- modify nseconds to 10
This will successfully run exercise_7_6
Can use this for Unit 8 Quiz
"""
...
ardron_publisher = rospy.Publisher('/cmd_vel', Twist , queue_size=1)
ardron_takeoff = rospy.Publisher('/drone/takeoff', Empty, queue_size=1)
ardron_land = rospy.Publisher('/drone/land', Empty, queue_size=1)
takeoff = Empty()
landing = Empty()
move_drone = Twist()
...
while status < DONE:
    ardron_takeoff.publish(takeoff)
    move_drone.linear.x = 0.5
    move_drone.angular.z = 0.5
    while ardron_publisher.get_num_connections() < 1:
        # wait for a connection to publisher
        rospy.sleep(1)


    ardron_publisher.publish(move_drone)
    status = client.get_state()
